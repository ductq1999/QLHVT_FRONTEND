<template>
  <card>
    <h5 slot="header" class="title">Sửa chuyến xe</h5>
    <form>
      ...
      <button class="btn btn-primary" @click="updateTrip()">Submit</button>
...
export default {
  ...
  methods: {
    checkForm(e) {
     ...
     async getTripById() {
      await this.$axios
        .$get("trip/" + this.$route.params.id)
        .then((response) => {
          if (response.code === 200) {
            this.$store.dispatch("trip/setTripByIdAction", response.data);
    ...
    async updateTrip() {
      if (this.checkForm()) {
        ...
        await this.$axios.$put("trip/update", data).then((response) => {
          if (response.code === 200) {
